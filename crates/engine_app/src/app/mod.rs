...
impl FrameProfiler {
    ...
    fn print_summary(&mut self) {
        ...
        println!("\n📊 Performance Summary:");
        println!(
            "   FPS: {:.1} (avg: {:.2}ms, min: {:.2}ms, max: {:.2}ms) {} slow frames",
        ...
        println!(
            ...
            self.visible_chunks,
            self.culled_chunks,
            self.chunks_unloaded
        );
        println!(
            "   Rendering: {} triangles, {} draw calls",
            self.rendered_triangles, self.draw_calls
        );
        println!(
            "   Jobs: {} pending, {} completed/frame",
...
impl App {
    pub fn new() -> Self {
        ...
        println!(
            "   • Render distance: {:.0} blocks",
            config.graphics.render_distance
        );
        println!("   • FOV: {:.0}°", config.graphics.fov_degrees);
        println!(
            "   • Worker threads: {} (rayon global pool)",
        ...
        println!(
            "   • View radius: {} chunks spherical (~{} chunks loaded, {} blocks radius)",
            view_radius,
    ...
    fn display_fps_stats_and_exit(&self, el: &ActiveEventLoop) {
        ...
        if self.fps_sample_count > 0 {
            ...
            println!(
                "Average FPS: {:.1} (based on {} samples)",
                avg_fps, self.fps_sample_count
            );
        } else {
...
impl App {
    ...
    fn upload_chunk_mesh(&mut self, position: IVec3, mesh: voxel_engine::MeshData) {
        if let Some(g) = self.gfx.as_mut() {
            ...
            let vertices: Vec<VertexTex> = mesh
                .positions
                .iter()
                .zip(mesh.uvs.iter())
                .map(|(p, u)| {
    ...
    fn handle_voxel_picking(&mut self, button: rwinit::event::MouseButton) {
        if let Some(g) = self.gfx.as_ref() {
            ...
            if let Some(hit) = self
                .chunk_manager
                .raycast(cam_pos, cam_forward, max_distance)
            {
                match button {
                    rwinit::event::MouseButton::Left => {
                        // Left click: REMOVE block (mine)
                        println!(
                            "⛏️ Mining block at {:?} (was {:?})",
                            hit.position, hit.block_id
                        );
                    ...
                    rwinit::event::MouseButton::Right => {
                        ...
                        println!(
                            "🧱 Placing block at {:?} (adjacent to {:?})",
                            hit.adjacent_position, hit.position
                        );
...
impl ApplicationHandler for App {
    ...
    fn resumed(&mut self, el: &ActiveEventLoop) {
        if self.window.is_none() {
            let mut attrs =
                Window::default_attributes().with_title("Dev Engine - Voxel World (NEW)");
            ...
            println!(
                "View radius: {} chunks",
                self.config.calculate_view_radius()
            );
            println!(
                "Render distance: {:.0} blocks",
                self.config.graphics.render_distance
            );
    ...
    fn about_to_wait(&mut self, _el: &ActiveEventLoop) {
        ...
        while now >= self.next_tick {
            if let Some(g) = self.gfx.as_mut() {
                ...
                let forward = (self.input.held(KeyCode::KeyW) as i32
                    - self.input.held(KeyCode::KeyS) as i32) as f32;
                let right = (self.input.held(KeyCode::KeyD) as i32
                    - self.input.held(KeyCode::KeyA) as i32) as f32;
                let up = (self.input.held(KeyCode::Space) as i32
