...
mod tests {
    ...
    fn test_aabb_creation() {
        let aabb = AABB::from_chunk_pos(1, 0, 2, 32.0);
        assert_eq!(aabb.min, Vec3::new(32.0, 0.0, 64.0));
        assert_eq!(aabb.max, Vec3::new(64.0, 32.0, 96.0));
        assert_eq!(aabb.center(), Vec3::new(48.0, 16.0, 80.0));
    }
    ...
    fn test_frustum_culling() {
        ...
        let aabb_origin = AABB::new(Vec3::new(-1.0, -1.0, -1.0), Vec3::new(1.0, 1.0, 1.0));
        assert!(frustum.test_aabb(&aabb_origin), "Origin AABB should be visible");
        
        // AABB far away should not be visible
        let aabb_far = AABB::new(Vec3::new(200.0, 0.0, 0.0), Vec3::new(210.0, 10.0, 10.0));
        assert!(!frustum.test_aabb(&aabb_far), "Far AABB should not be visible");
    }
}
