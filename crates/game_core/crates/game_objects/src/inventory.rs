...
impl Inventory {
    ...
    pub fn max_volume_to_add(&self, item: &Item, quantity: u32) -> u32 {
        (((self.max_volume - self.current_volume()) / &item.volume_per_unit).floor() as u32).min(quantity)
    }

    pub fn max_to_add(&self, item: &Item, quantity: u32) -> u32 {
        self.max_mass_to_add(&item, quantity).min(self.max_volume_to_add(&item, quantity))
    }
