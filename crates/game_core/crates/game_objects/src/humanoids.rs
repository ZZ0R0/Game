...
impl Humanoid {
    ...
    pub fn compute_and_apply_pending_deltas(&mut self) -> Option<HumanoidDelta> {
        ...
        let merged = HumanoidDelta::merge(self.pending_deltas.clone());
    ...
    pub fn create_position_delta(&self, new_position: FloatPosition, timestamp: u64, sequence: u64) -> HumanoidDelta {
        HumanoidDelta {
            player_id: self.id.0,
    ...
    pub fn create_resources_delta(&self, timestamp: u64, sequence: u64) -> HumanoidDelta {
        HumanoidDelta {
            player_id: self.id.0,
            position: None,
            orientation: None,
...
/// 
/// Le HumanoidDelta contient tous les aspects qui peuvent changer pour un joueur :
...
impl HumanoidDelta {
    ...
    pub fn apply_to(&self, player: &mut Humanoid) {
    ...
    pub fn merge(deltas: Vec<HumanoidDelta>) -> Option<HumanoidDelta> {
        if deltas.is_empty() {
            return None;
        }
