...
mod tests {
    ...
    fn test_world_to_chunk() {
        assert_eq!(world_to_chunk(Vec3::new(0.0, 0.0, 0.0)), IVec3::new(0, 0, 0));
        assert_eq!(world_to_chunk(Vec3::new(32.0, 0.0, 0.0)), IVec3::new(1, 0, 0));
        assert_eq!(world_to_chunk(Vec3::new(-1.0, 0.0, 0.0)), IVec3::new(-1, 0, 0));
        assert_eq!(world_to_chunk(Vec3::new(100.0, 50.0, 200.0)), IVec3::new(3, 1, 6));
    }
    
    #[test]
    fn test_chunk_ring_update() {
        let config = ChunkRingConfig {
            view_radius: 2,
            generation_radius: 3,
            unload_radius: 4,
        };
        
        let mut ring = ChunkRing::new(config);
        
        // Initial update at origin
        let (to_load, to_unload) = ring.update(Vec3::ZERO);
        assert!(!to_load.is_empty());
        assert!(to_unload.is_empty());
        
        // Mark chunks as loaded
        for chunk in &to_load {
            ring.mark_loaded(*chunk);
        }
        
        // Move to same chunk - no changes
        let (to_load, to_unload) = ring.update(Vec3::new(10.0, 0.0, 10.0));
        assert!(to_load.is_empty());
        assert!(to_unload.is_empty());
        
        // Move far away - should have loads and unloads
        let (to_load, to_unload) = ring.update(Vec3::new(200.0, 0.0, 200.0));
        assert!(!to_load.is_empty());
        assert!(!to_unload.is_empty());
    }
}
