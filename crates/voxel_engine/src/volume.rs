...
use crate::voxel_schema::{VoxelSchema, MaterialId};
use crate::chunk::{CHUNK_SIZE, BlockId};
...
impl DirtyRegions {
    ...
    pub fn mark_position_dirty(&mut self, world_pos: IVec3) {
        ...
        if on_boundary_x {
            self.dirty_chunks.insert(chunk_pos + IVec3::new(if local.x == 0 { -1 } else { 1 }, 0, 0));
        }
        if on_boundary_y {
            self.dirty_chunks.insert(chunk_pos + IVec3::new(0, if local.y == 0 { -1 } else { 1 }, 0));
        }
        if on_boundary_z {
            self.dirty_chunks.insert(chunk_pos + IVec3::new(0, 0, if local.z == 0 { -1 } else { 1 }));
        }
    }
...
impl Volume for GridVolume {
    ...
    fn set_voxel(&mut self, world_pos: IVec3, _block: BlockId) -> bool {
        ...
        if let Some((min, max)) = self.bounds {
            if chunk_pos.x < min.x || chunk_pos.x > max.x
