...
impl JobQueue {
    ...
    /// Process all pending jobs immediately using Rayon (called from main thread)
    /// Returns number of jobs processed
    pub fn process_all_jobs(&self) -> usize {
        let jobs = {
            let mut pending = self.pending.lock().unwrap();
            let all_jobs = pending.drain(..).collect::<Vec<_>>();
            all_jobs
        };
        ...
        // Process jobs directly with Rayon (no intermediate worker threads!)
